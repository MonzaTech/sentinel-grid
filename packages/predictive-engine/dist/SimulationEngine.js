...
exports.WARNING_THRESHOLD = exports.CRITICAL_THRESHOLD = void 0;
...
// ============================================================================
exports.CRITICAL_THRESHOLD = 0.8;
exports.WARNING_THRESHOLD = 0.6;
const NODE_TYPES = [
    'substation',
    'transformer',
    'generator',
...
function initializeNodes(config = {}) {
    ...
    const rng = new SeededRandom_1.SeededRandom(cfg.seed);
    const nodes = {};
    ...
    nodeIds.forEach((nodeId) => {
        ...
        const sameRegion = nodeIds.filter((id) => id !== nodeId && nodes[id].region === node.region);
        const otherRegion = nodeIds.filter((id) => id !== nodeId && nodes[id].region !== node.region);
...
function fetchWeatherData(rng) {
    const r = rng || new SeededRandom_1.SeededRandom(Date.now());
    const conditions = [
...
function updateNodeState(nodes, threat, weather, config = {}) {
    const cfg = { ...types_1.DEFAULT_CONFIG, ...config };
    const rng = new SeededRandom_1.SeededRandom(Date.now());
    const updatedNodes = {};
    ...
    if (weather.condition === 'storm')
        ...
        weatherImpact = 1.5;
    else if (weather.condition === 'extreme_cold')
        weatherImpact = 1.3;
    ...
    Object.entries(nodes).forEach(([nodeId, node]) => {
        ...
        if (weather.condition === 'extreme_heat') {
            ...
            if (node.type === 'telecom_tower')
                newRisk += 0.03;
        }
        // Active threat effects
        if (threat && threat.active) {
            const isThreatTarget = threat.target === nodeId ||
        ...
        if (neighborRisks.length > 0) {
            ...
            if (avgNeighborRisk > exports.CRITICAL_THRESHOLD) {
        ...
        // Determine status
        let status = 'online';
        if (newRisk > exports.CRITICAL_THRESHOLD || newHealth < 0.3) {
            status = 'critical';
        }
        else if (newRisk > exports.WARNING_THRESHOLD || newHealth < 0.6) {
            status = 'degraded';
        }
        updatedNodes[nodeId] = {
            ...node,
...
function simulateCascade(nodes, originId, severity = 0.7, config = {}) {
    ...
    while (queue.length > 0) {
        ...
        for (const connId of node.connections) {
            ...
            if (!rng.nextBool(propagationChance))
                continue;
            visited.add(connId);
            ...
            updatedNodes[connId] = {
                ...
                riskScore: newRisk,
                health: newHealth,
                status: newRisk > exports.CRITICAL_THRESHOLD ? 'critical' :
                    newRisk > exports.WARNING_THRESHOLD ? 'degraded' : connNode.status,
            };
    ...
    const event = {
        ...
        startTime: new Date(),
        propagationPath,
        totalDamage: affectedNodes.reduce((sum, id) => sum + (updatedNodes[id].riskScore - nodes[id].riskScore), 0),
    };
...
function autoMitigate(nodes, nodeId, config = {}) {
    ...
    if (node.riskScore > exports.CRITICAL_THRESHOLD) {
    ...
    const updatedNode = {
        ...
        status: (node.riskScore - riskReduction) > exports.WARNING_THRESHOLD ? 'degraded' : 'online',
    };
    return {
...
function getSystemState(nodes) {
    ...
    return {
        ...
        criticalNodes: nodeList
            .filter((n) => n.riskScore > exports.CRITICAL_THRESHOLD || n.status === 'critical')
            .map((n) => n.id),
        warningNodes: nodeList
            .filter((n) => n.riskScore > exports.WARNING_THRESHOLD && n.riskScore <= exports.CRITICAL_THRESHOLD)
            .map((n) => n.id),
...
function createSignedHash(data, hmacKey) {
    ...
    const signature = (0, crypto_1.createHmac)('sha256', hmacKey).update(sha256).digest('hex');
