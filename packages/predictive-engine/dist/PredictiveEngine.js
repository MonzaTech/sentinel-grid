...
const DEFAULT_PREDICTIVE_CONFIG = {
    seed: 54321,
...
class PredictiveEngine {
    constructor(config = {}) {
        ...
        this.rng = new SeededRandom_1.SeededRandom(this.config.seed);
    }
    // ==========================================================================
    // History Management
    // ==========================================================================
    ...
    analyzePatterns(nodes) {
        ...
        const criticalNodes = nodeList.filter((n) => n.riskScore > SimulationEngine_1.CRITICAL_THRESHOLD);
        criticalNodes.forEach((critical) => {
            const connectedAtRisk = critical.connections.filter((connId) => {
                const conn = nodes[connId];
                return conn && conn.riskScore > SimulationEngine_1.WARNING_THRESHOLD;
            });
    ...
    generatePredictions(nodes) {
        ...
        nodeList.forEach((node) => {
            ...
            if (history && history.riskScores.length >= 5) {
                ...
                if (trend > 0.01) {
                    const projectedTime = (SimulationEngine_1.CRITICAL_THRESHOLD - node.riskScore) / velocity;
            ...
            if (node.riskScore > SimulationEngine_1.WARNING_THRESHOLD) {
                const riskProbability = 0.4 + (node.riskScore - SimulationEngine_1.WARNING_THRESHOLD) * 2;
                maxProbability = Math.max(maxProbability, riskProbability);
                if (node.riskScore > SimulationEngine_1.CRITICAL_THRESHOLD) {
                ...
                factors.push({
                    ...
                    threshold: SimulationEngine_1.WARNING_THRESHOLD,
                    trend: 'increasing',
                });
            }
            ...
            const connectedRisks = node.connections
                ...
                .filter((r) => r > SimulationEngine_1.WARNING_THRESHOLD);
    ...
    getSystemHealthScore(nodes) {
        ...
        const urgentPredictions = this.predictions.filter((p) => p.hoursToEvent < 12 && p.probability > 0.7);
        const predictionPenalty = Math.min(0.3, urgentPredictions.length * 0.05);
        // Penalty for patterns
        const escalatingPatterns = this.detectedPatterns.filter((p) => p.trend === 'escalating');
    ...
    getAccuracyMetrics() {
        ...
        types.forEach((type, idx) => {
            ...
            const typeAccurate = this.resolvedPredictions.filter((p) => p.type === type && p.wasAccurate).length;
    ...
    getSeverity(probability, hoursToEvent) {
        if (probability > 0.85 || hoursToEvent < 4)
            return 'critical';
        if (probability > 0.7 || hoursToEvent < 8)
            return 'high';
        if (probability > 0.55 || hoursToEvent < 16)
            return 'medium';
        return 'low';
    }
    ...
    determineTrend(nodes, metric) {
        ...
        if (escalating > resolving * 1.5)
            return 'escalating';
        if (resolving > escalating * 1.5)
            return 'resolving';
        return 'stable';
    }
    calculateRegionRisks(nodes) {
        ...
        return Object.fromEntries(Object.entries(regionTotals).map(([region, { sum, count }]) => [
            region,
            sum / count,
        ]));
    }
...
exports.default = PredictiveEngine;
