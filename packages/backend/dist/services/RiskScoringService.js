...
exports.predictCascadePath = predictCascadePath;
...
exports.clearAccuracyStats = clearAccuracyStats;
...
function calculateRiskScore(node) {
    ...
    const cascadingRisk = neighborRisks.length > 0
        ...
        : 0;
    // Overall risk
    const overall = Math.min(1, physicalRisk * 0.3 +
        cyberRisk * 0.25 +
        operationalRisk * 0.2 +
...
// Cascade Prediction
// ============================================================================
function predictCascadePath(originNodeId) {
    const visited = new Set();
    const path = [originNodeId];
    const queue = [];
...
function generatePrediction(node) {
    ...
    const matchedPattern = HISTORICAL_PATTERNS.find(p => {
        if (predictionType === 'thermal_stress' && p.id === 'pattern-thermal-cascade')
            return true;
        if (predictionType === 'cyber_vulnerability' && p.id === 'pattern-cyber-propagation')
            return true;
        if (predictionType === 'overload' && p.id === 'pattern-overload-cascade')
            return true;
        return false;
    });
    ...
    const contributingFactors = riskScore.leadingFactors.slice(0, 5).map(f => ({
        factor: f.name,
    ...
    // Severity level
    const severityLevel = riskScore.overall >= 0.8 ? 'critical' :
    ...
    return {
        id: `pred_${(0, uuid_1.v4)().slice(0, 8)}`,
        nodeId: node.id,
        nodeName: node.name,
        type: predictionType,
...
// ============================================================================
function generateMitigationText(predictionType, node) {
...
// ============================================================================
function generateSuggestedActions(predictionType, node, riskScore) {
    const actions = [];
...
function recordPredictionOutcome(predictionId, wasAccurate, type) {
...
//# sourceMappingURL=RiskScoringService.js.map